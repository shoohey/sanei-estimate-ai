...
- planned_panels（設置枚数）: 通常 30〜2000枚。100枚単位や288/376/66/40枚など
  ...
  ※この「典型値に寄せる」推測は **手書き数字の判読補助** であり、印字・CADで
  作成された自社作成図面に記載の枚数には適用しない（図面の印字値は半端な枚数でも
  そのまま採用する）。
...
- 配置図/屋根伏図: パネル配置・離隔距離・パネル枚数
  （印字・CADで作成された自社作成図面なら、枚数・配置・容量はこれを正とする）

【書類の役割分担と情報源の優先順位（重要・必ず適用）】
入力は「手書き現調シート ＋ 自社作成図面」の組み合わせで渡されることが多い。
- 自社作成図面とは: CAD・印字で作成されたパネルレイアウト図・配置図・屋根伏図。
  文字が印字フォントで、直線・寸法線・パネル格子が機械的に整っている図面を指す。
  手書き記入が主体のフォーム（現調シート）とは視覚的に区別すること。
- 自社作成図面が含まれる場合、パネル枚数（planned_panels）・パネル配置・
  PV容量（pv_capacity_kw）は **図面の記載を正** とする。図面は枚数確定済みの
  設計成果物であり、手書き現調シートの記入より優先する。
- 手書き現調シートは、設備仕様（メーカー・型式・モジュール出力）、現場条件
  （接地・VT/CT・クレーン・足場・電柱・配線ルート・キュービクル等）、
  チェック項目・最終確認欄の読み取りに使う。
- 図面と手書き現調シートで値が食い違う場合は図面の値を採用し、
  extraction_warnings にその食い違いを必ず明記する。
  例: 「枚数は自社図面から採用（328枚）。手書き現調シートには320枚と記載あり要確認」
- 自社作成図面（印字値）から採用したフィールドの field_confidences は "high" とする
  （例: equipment.planned_panels: "high"）。
...
- 屋上配置図 / 屋根伏図 / パネルレイアウト図（自社作成図面）: CAD・印字で作成された
  設計図面。含まれる場合、パネル枚数・パネル配置・PV容量はこの図面を正とする

これらすべてから情報を統合し、後述のJSON形式で返してください。
自社作成図面（印字レイアウト図）がある場合は枚数・配置・容量を図面から読み取り、
手書き現調シートは設備仕様・現場条件・チェック項目の読み取りに使うこと。
{_COMMON_HEURISTICS}
...
- 屋根伏図 / パネルレイアウト図（自社作成図面）: パネル配置図、回路構成、ケーブル経路。
  CAD・印字で作成された図面が含まれる場合、パネル枚数・配置はこの図面を正とする
